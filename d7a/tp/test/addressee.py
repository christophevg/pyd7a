...
class TestAddressee(unittest.TestCase):
  ...
  def test_id_property(self):
    addr = Addressee(vid=True, id=0x1234)
  ...
  def test_addressee_id_is_positive_value(self):
    def bad(): addr = Addressee(vid=True,id=-1)
    self.assertRaises(ValueError, bad)
    def bad(): addr = Addressee(vid=False,id=-1)
    self.assertRaises(ValueError, bad)

  def test_virtual_addressee_id_consists_of_max_2_bytes(self):
    def bad(): addr = Addressee(vid=True,id=0x1FFFF)
    self.assertRaises(ValueError, bad)

  def test_universal_addressee_id_consists_of_max_8_bytes(self):
    def bad(): addr = Addressee(vid=False,id=0x1FFFFFFFFFFFFFFFF)
    self.assertRaises(ValueError, bad)
  ...
  def test_byte_generation(self):
    ...
    bs = bytearray(Addressee(vid=True, id=0x1234))
    ...
    bs = bytearray(Addressee(id=0x1234567890123456))
